 ...
 SUBROUTINE link_TOV(mout,r2out,xx1,d1,x2,d2,stp,yLout,nn,bb)
  ...
  IF(Love) THEN
   ...
   DO i=1,linkdots
    ...
    pp=rhos*EXP(xo)
    RHS=r2o*(r-2*mo)/(mo+4*pi*r**3*pp)
    dmdx=-4*pi*(1-fo)*r*RHS*pp
    dr2dx=-fo*2*RHS 
 
    El=1/(1-2*mo/r)
    rr=rhos*EXP(yo)
    nup=2*El*(mo+4*pi*r**3*pp)/r2o
    Q=4*pi*El*(5*rr+9*pp+(rr+pp)*rr*(alph+1/(1+EXP(x1-xo)*bet))/pp)-6*El/r2o-nup**2
    W=El*(1+4*pi*r2o*(pp-rr))
    dyLdx=(yLo*yLo+yLo*W+r2o*Q)*(r-2*mo)*fo/(mo+4*pi*r**3*pp)
    ...
    pp=rhos*EXP((xx(i)+xo)/2)
    RHS=r2out*(r-2*mout)/(mout+4*pi*r**3*pp)
    dmdx=-4*pi*(1-(ff(i)+fo)/2)*r*RHS*pp
    dr2dx=-(ff(i)+fo)*RHS

!    dmdx=(-4*pi*(1-(ff(i)+fo)/2)*r*RHS*pp)/2
!    dr2dx=(dr2dx-ff(i)*2*RHS)/2

    El=1/(1-2*mout/r)
    rr=rhos*EXP(yy(i))
    nup=2*El*(mout+4*pi*r**3*pp)/r2out
    Q=4*pi*El*(5*rr+9*pp+(rr+pp)*rr*(alph+1/(1+EXP(x1-xx(i))*bet))/pp)-6*El/r2out-nup**2
    W=El*(1+4*pi*r2out*(pp-rr))
    dyLdx=(dyLdx+(yLout**2+yLout*W+r2out*Q)*(r-2*mout)*ff(i)/(mout+4*pi*r**3*pp))/2
...
!    WRITE(*,*) 'Error stp=0: decreasing mass or radius profile.'
  ...
  ELSE
   DO i=1,linkdots
    ...
    r2out=r2o+dr2dx*dx
    IF(mout<mo.OR.r2out<r2o) THEN
!    WRITE(*,*) 'Error stp=0: decreasing mass or radius.'
     stp=0 
     RETURN
    ENDIF
    mo=mout
...
!$OMP CRITICAL
   ...
   ENDIF
!$OMP END CRITICAL
!   IF(Mns>0.AND.Mns<=MR) mMaxAll=MAX(Mns,mMaxAll)
  ENDDO
!$OMP END PARALLEL DO
...
!  lnk=(str-2)/linkpoints
  ...
  bstr=0
  IF(BE) bstr=(r2str*EXP(nstr)/SQRT(1+2*mstr/SQRT(r2str)))*SQRT(r2str)/2
  ...
  IF(MomI) THEN
   ...
   profile(iseed+str-1,3,str)=mstr
   profile(iseed+str-1,4,str)=SQRT(r2str)
  ENDIF
  i=str-1
  DO WHILE(i>=2)
   ...
   IF(MomI) THEN
    ...
    profile(iseed+i-1,3,str)=mstr
    profile(iseed+i-1,4,str)=SQRT(r2str)
   ENDIF
   i=i-1
  ENDDO
  IF(NSchain(1,1)==0) THEN ! if it bare SQM star, and surface is reached
   NSchain(str,1)=mstr/Rkm
   NSchain(str,2)=SQRT(r2str)
   IF(nDen) NSchain(str,7)=nseed*EXP(-nstr)
   IF(BE) NSchain(str,8)=mAc2*4*pi*nseed*EXP(-nstr)*bstr*toMsc2-mstr/Rkm
   IF(Love) THEN
  ...
  IF(MomI) THEN
    profile(iseed,3,str)=mstr
    profile(iseed,4,str)=SQRT(r2str)
  ENDIF
  IF(stp<=0) RETURN
  ...
  DO WHILE(i>1)
   ...
   IF(stp<=0) RETURN
   IF(MomI) THEN
    profile(i-1,3,str)=mstr
    profile(i-1,4,str)=SQRT(r2str)
   ENDIF
   i=i-1
  ENDDO
  NSchain(str,1)=mstr/Rkm
  ...
  IF(nDen) NSchain(str,8)=4*pi*mAc2*nEOS(iseed)*EXP(-nstr)*bstr*toMsc2-mstr/Rkm
  IF(Love) THEN
   CALL calc_lambda(mstr,NSchain(str,2),yLstr,lam)
   IF(lam.NE.lam) THEN
     NSchain(str,5)=-1
...
!$OMP END PARALLEL DO
  ...
  bstr=0
  IF(BE) bstr=r2str*EXP(nstr)*SQRT(r2str)/(2*SQRT(1+2*mstr/SQRT(r2str)))
  ...
  DO i=s,2,-1
   ...
   IF(MomI) THEN
    profile(i-1,3,1)=mstr
    profile(i-1,4,1)=SQRT(r2str)
   ENDIF
  ENDDO
  NSchain(1,1)=mstr/Rkm ! in M_Sun
  NSchain(1,2)=SQRT(r2str) ! in km
  NSchain(1,3)=xs ! rhos*EXP(xseed)/EDtoKM ! in MeV/fm^3
  NSchain(1,4)=ys ! rhos*EXP(yseed)/EDtoKM ! in MeV/fm^3
  IF(nDen) NSchain(1,7)=nEOS(s)*EXP(-nstr)
  IF(BE) NSchain(1,8)=nEOS(s)*EXP(-nstr)*4*pi*mAc2*bstr*toMsc2-mstr/Rkm
  IF(MomI) THEN
   CALL calc_I(1)
   NSchain(1,6)=profile(1,5,1)/Rkm ! in M_Sun*km^2
  ENDIF
  IF(Love) THEN
   IF(dyR/=0) yL=yL-dyR*rhos*EXP(xEOS(1))*4*pi*r2str**(3.0D0/2.0D0)/mstr
   CALL calc_lambda(mstr,NSchain(1,2),yL,lam)
   IF(lam.NE.lam) THEN
     NSchain(1,5)=-1
   ELSE
     NSchain(1,5)=lam
